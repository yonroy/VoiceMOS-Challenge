...
> ⚠️ **Không commit 3 thư mục này** vào repo tài liệu của mình (chúng là repo git riêng + nặng). Thêm vào `.gitignore`.
...
| **Track 2** ⭐ | Emotional TTS | xem bảng dưới | Trong `vmc2026-baselines/track2/` | ⏳ chờ data (QMOS/EmoCat test ESD được) |
| Track 3 | Codec-based synthesis | ECAPA-TDNN | `vmc2026-baselines/track3/` (có code + **checkpoint pre-trained**) | ⏳ setup được, chạy chờ data license |
...
- ⚠️ Cần copy wav từ phân phối `_vctk` sang `_syn/wav/` (xem README track3).
...
`vmc2026-baselines/track2/EmoCat/run_vmc2026.py`
```bash
...
Code nằm sẵn trong `vmc2026-baselines/track2/EMOS/` và `.../VAD/`.
...
**Trạng thái chạy được ngay (chưa cần data thật):** QMOS + EmoCat test trên ESD (Kaggle dataset). EMOS/VAD chờ data Track 2 (cần `metadata.csv` chứa nhãn cảm xúc target).
