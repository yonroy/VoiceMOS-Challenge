...
Hướng dẫn dựng môi trường để reproduce được toàn bộ project. Cập nhật ngày: 9/6/2026.

---

## 📦 Kaggle Datasets của dự án (Add Input → slug)

| Slug | Nội dung | Dùng cho |
|---|---|---|
| `toanminh222/cache-exp8` | **Checkpoint cảm xúc tốt nhất** `archive/ft_emotion_full_20epoch.pt` + cache audeering | exp13 (nạp 5 cột cảm xúc), inference exp08, phase Evaluation |
| `nguyenthanhlim/emotional-speech-dataset-esd` | ESD (mirror cộng đồng) | ráp data Track 2 |
| `phngtvit/dailytalk` | DailyTalk (mirror cộng đồng) | ráp data Track 2 |

> Đường dẫn mount checkpoint cảm xúc trên Kaggle:
> `/kaggle/input/datasets/toanminh222/cache-exp8/archive/ft_emotion_full_20epoch.pt`
> ⚠️ Đây là bản TỐT NHẤT (`_20epoch`), KHÔNG dùng `ft_emotion_full.pt`. Upload 9/6 (Phiên 14) để chống mất khi kernel chết.
