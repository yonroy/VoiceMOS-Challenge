...
## 🧠 Buổi học Phiên 23 (11/6) — SRCC tính tay · từng layer 3 track · CNN→Transformer có toán · pretrain/fine-tune · ranking loss trong code

> Buổi học dài nhất từ đầu dự án — toàn bộ đã kết tinh vào `docs/22_slides_v2_paper_style.md` (slide) + `slide/model_design_menu.md` (thực đơn thiết kế). Dưới đây là phần cô đọng.

1. **SRCC phạt theo d² (khoảng cách lệch hạng), KHÔNG đếm số câu sai:** cùng "đúng 3/5 câu" → đảo cặp liền kề = 0.90, đảo lệch 2 bậc = 0.60, đảo nhất↔bét = **−0.60**. Hệ quả: pairwise margin loss (phạt mọi cặp như nhau) lệch với SRCC — lý do tồn tại các loss listwise. Hai tầng điểm: mỗi audio 1 điểm (trung bình nhiều người nghe → số lẻ) ≠ điểm leaderboard (1 số SRCC cho CẢ model trên toàn tập; chỉ so được khi cùng bộ audio).
2. **Ranking loss = đổi xếp-hạng-toàn-cục thành so-cặp:** hợp lệ vì "đúng cả danh sách ⇔ đúng mọi cặp" (mục tiêu phân rã được); thực tế chỉ học mẫu cặp ngẫu nhiên trong batch/cửa sổ. **Cần ≥2 mẫu "sống" trong 1 lần backward**: BATCH=1 thuần → 0 cặp (chết); mẹo exp13 = gom cửa sổ ACCUM 16 → C(16,2)=120 cặp (giá = VRAM giữ đồ thị cả cửa sổ). **Soi code:** exp13 kỷ lục 0.6296 = MSE thuần (λ=0, margin=0, không lọc cặp nhiễu); exp15 bật λ=0.3 cho 4 cột SRCC (CAT giữ soft-CE) nhưng BATCH=2 → 1 cặp/forward (yếu).
3. **CNN 7 lớp (kernel/stride):** các lớp nối TIẾP (lớp sau ăn đầu ra lớp trước, receptive field rộng dần), tổng nén = tích stride = 5·2⁶ = 320×; "khối đặc trưng" L×512 = thời gian × KÊNH (512 filter cùng tầng, không phải 512 lớp). **Transformer = contextual embedding:** không đổi chiều không gian, chỉ "dời tọa độ" từng khung theo ngữ cảnh — ví dụ toán tính tay: 2 khung input giống hệt [1,0] (xì /s/ vs xì nhiễu), sau attention (dot + thiên vị vị trí + softmax + trung bình có trọng số) ra [0.65, 0.18] vs [0.77, 0.03] — ngữ cảnh chui vào vector chỉ bằng nhân-cộng. Ẩn dụ: "cuộc họp công ty" (Q=câu hỏi, K=danh thiếp, V=nội dung, softmax=chia 100% sự chú ý, residual=không xé sổ cũ, ×24 vòng).
4. **Track 1 URGENT-MOS từng layer:** 4 encoder ❄ khác "bài tập thời học nghề" (WavLM=kỹ sư âm thanh học che-đoán-giữa-nhiễu; Kimi/Qwen/Flamingo=audio-LLM cưa lấy nửa encoder, output = vector vài nghìn số KHÔNG tự giải thích — head "thư ký" học bảng tra tương quan ô↔đáp án qua nhãn, không "hiểu" gì) → align+concat → AMPM (hồi quy ACR [1,5]) + NCPM (**hiệu 2 nhánh chung trọng số** g(f_A)−g(f_B) → CCR [−3,+3] = thang 7 mức chuẩn ITU so-cặp, đối xứng quanh 0 khớp phép hiệu). **Trộn lớp Σαₗ·H⁽ˡ⁾** = cách chọn tầng khi không-được-fine-tune (24 tham số, đáng thử cho exp08).
5. **Track 3 ECAPA từng layer (đọc code `model.py`):** Mel fbank (không ăn sóng thô) → TDNN giãn nở (`h[t]=Σw[k]x[t+d·k]` — cùng 3 trọng số, tầm nhìn ×d) → SE-block (squeeze→excite→scale: hệ số kênh tính từ chính câu đang nghe) → **attentive stat-pool [μ‖σ]** (giữ "độ rung" — 2 giọng cùng μ khác σ phân biệt được; mean-pool thì không) → Linear → ℝ¹⁹² → L2-norm → cosine. Bẫy hay: lớp projection **chưa-train phải tắt** ở zero-shot (trọng số random = máy xáo bài). Fine-tune path: interaction `[a;b;|a−b|;a⊙b]` → MLP → Tanh×2+3.
6. **WavLM "bịt tai" (masked prediction):** che span ~200ms (gần nửa câu) bằng mask embedding → đoán **số hiệu cụm k-means** của khung bị che (nhãn-giả, không cần người chấm) → cross-entropy chỉ tại chỗ che; WavLM thêm chiêu **vừa che vừa phá** (trộn nhiễu/giọng đè, đáp án vẫn là cụm bản sạch) → "thính" nhiễu. Thắng trò chơi buộc phải hiểu quy luật tiếng nói → kiến thức kết tinh vào hidden states.
7. **Pretrain vs fine-tune vs scratch:** pretrain = chặng ĐẦU từ scratch trên nhiệm vụ tổng quát/data khổng lồ (chỉ lab lớn làm); SAILER được tạo bằng fine-tune (= *intermediate fine-tuning*) nhưng đóng vai *pretrained checkpoint* với mình — "pretrain" là vai trò tương đối. Fine-tune > frozen > scratch khi: có nhãn domain đủ + LR nhỏ + mở đúng phần + **fine-tune đúng model cho đúng cột** (exp08 giỏi cảm xúc nhưng QMOS xẹp; exp13 fine-tune UTMOS mới phá QMOS — "mỗi cột một khẩu vị").
8. **Kiến trúc là HỆ QUẢ của (nhiệm vụ, dữ liệu, tài nguyên)** — không ai vẽ kiến trúc trước rồi tìm data; mọi quyết định exp08 truy được về 1 sự thật ở tầng trên (6 cột→multi-head; nhãn EMOS so khớp→one-hot 517; 12.7k nhãn→mượn pretrain; T4→ACCUM). Checklist 5 câu + thực đơn linh kiện đầy đủ: `slide/model_design_menu.md`. **Zero-shot trong exp13** = 3 vai: đối chứng (chứng minh fine-tune có công), sàn an toàn (ckpt khởi đầu = zero-shot, chỉ ghi đè khi vượt), khám pipeline miễn phí trước khi đốt GPU.

---

## 🧠 Buổi học Phiên 22 (11/6) — SRCC khi triển khai thực tế · calibration · so 2 model · CAT-ERR

1. **SRCC cao ≠ value đúng:** SRCC chỉ đo thứ tự — nhãn `[2.0, 3.5, 4.8]` đoán `[1.1, 1.5, 1.9]` vẫn SRCC = 1.0. Hệ của mình lệch domain (100 audio tiếng Việt: QMOS ~1.85, EMOS trần ~3.4) nhưng ranking vẫn dùng được. **Việc chỉ cần ranking** (chiếm đa số use case): so sánh model A/B, reranking ("sinh 5 bản chọn bản tốt nhất"), regression test, vòng lặp emotional ruler. **Việc cần value thật:** báo "MOS 4.2" cho khách, ngưỡng release tuyệt đối, so điểm GIỮA 2 domain.
2. **Calibration chữa thang đo không cần train lại:** fit ánh xạ ŷ→y trên ~50–100 file domain đích có nhãn tay — linear (`y=a·ŷ+b`), isotonic (đơn điệu, mềm hơn), hay z-score matching (μ/σ — đã có trong ckpt). Mọi ánh xạ **đơn điệu tăng không đổi SRCC** → calibrate cho ứng dụng thật mà không ảnh hưởng leaderboard. Giới hạn: chỉ sửa thang đo, không sửa được ranking sai (ranking hỏng thì phải fine-tune — bài học exp13).
3. **So 2 model TTS (mỗi model N audio):** chấm cùng 1 bộ chấm → **trung bình ĐIỂM mỗi model** rồi so (system-level, đúng cách BTC tính SYS-SRCC); lệch value triệt tiêu vì 2 bên lệch như nhau. Trung bình hạng cũng được (bền với outlier hơn). Công bằng nhất: **cùng câu (paired)** + Wilcoxon signed-rank; 10 mẫu/model là ít nếu 2 trung bình sát nhau.
4. **CAT-ERR = (1/(N×5))·ΣᵢΣ_c|p̂ᵢ(c)−pᵢ(c)|** — MAE giữa phân bố vote thật của người nghe và softmax 5 lớp, trung bình mỗi ô; dải [0, 0.4] (mỗi hàng tổng=1 → sai tối đa 2/audio). exp08 = 0.1331 ≈ lệch 0.13 xác suất/ô. Khác SRCC: **value tuyệt đối CÓ nghĩa** → softmax "mềm, dè dặt" (happy ~0.4–0.6, không bao giờ 0.95) là LỢI THẾ; argmax neutral-bias không ảnh hưởng vì CAT-ERR so cả 5 số, không so argmax.
5. **Kiểm chứng thực nghiệm (100 audio, EMOS thật):** SRCC(EMOS, cat_happy) = 0.95, SRCC(EMOS, valence) = 0.97 → khi không truyền target, `cat_happy` là proxy EMOS rất tốt; head hồi quy phân tách rõ (1.87–3.36) dù argmax chỉ nhận 32/100 happy — củng cố phát hiện Phiên 20 "đầu phân loại bias, đầu hồi quy vẫn cảm nhận đúng".

---

## 🧠 Buổi học Phiên 21 (10/6) — kiến trúc từng layer exp08 · Mamba vs mean-pool · 4 khái niệm vận hành

> Bài giảng đầy đủ nằm trong chat Phiên 21; sơ đồ từng layer đã ghi vào mục exp_mix của `04_` + mục 2 Track 2 của `12_`. Dưới đây là phần cô đọng đáng nhớ.

1. **Kiến trúc exp08 = "2 tai → 1 não → 3 miệng":** WavLM (tai chính, fine-tune 6 lớp trên — cảm xúc nằm ở lớp trên; CNN + 18 lớp dưới đóng băng vì data nhỏ 12.7k) + audeering (tai phụ chuyên VAD, frozen+cache) → concat 2051-D → trunk MLP 512 (Linear=máy trộn có học, ReLU=phi tuyến, Dropout=chống ỷ lại) → 3 head. **Head EMOS phải nối one-hot cảm xúc target** vì câu hỏi của nó là "khớp cảm xúc ĐƯỢC YÊU CẦU không" — CAT/VAD chỉ tả cái đang nghe nên không cần đề bài.
2. **Mamba head vs mean-pool (exp15 vs exp08):** mean-pool = trung bình 400 frame → **mất thứ tự thời gian** ("đều đều rồi vọt cao" vs "cao đều" cho cùng trung bình). Mamba = đọc tuần tự có "sổ ghi chú" (hidden state), **selective** = hệ số quên/ghi tính từ chính frame đang đọc, O(n); exp15 còn thay bước gom cuối bằng **attention-pooling** (trung bình có trọng số học được). **Kết quả thật (NỘP 10/6): Mamba ≈ mean-pool** — thua sát nút EMOS/CAT/VAL, **thắng đúng Arousal** (+0.0045, kỷ lục 0.7978) vì arousal = năng lượng giọng biến thiên theo thời gian rõ nhất → temporal modeling chỉ giúp cột có dynamics. Một dòng ablation đẹp cho paper.
3. **z-score μ/σ trong checkpoint:** train học trên thang `z=(y−μ)/σ` (mọi task cùng cỡ → loss cân, học ổn); predict phải giải ngược `y=z·σ+μ`. μ/σ tính từ nhãn train của ĐÚNG lần train đó → **bắt buộc lưu trong ckpt** (predict-only không có train.csv để tính lại). Lệch tuyến tính không sập SRCC (chấm thứ hạng) nhưng hỏng CAT-err + mọi diễn giải tuyệt đối.
4. **`load_state_dict` thiếu/dư key:** state_dict = từ điển tên-tensor; "thiếu" = model có chỗ mà ckpt không có trọng số (chỗ đó thành random → **sai âm thầm, không crash** vì strict=False); "dư" = trọng số bị vứt. `thiếu 0/dư 0` = kiến trúc dựng lại khớp 100% — nhưng chỉ xác nhận **hình dạng**, chất lượng phải nhìn `val_emos` lưu kèm ckpt.
5. **Vì sao "load" không ăn GPU:** nghẽn ở mạng (HF download, rate-limit khi thiếu HF_TOKEN) + disk + CPU unpickle; `map_location="cpu"` là cố ý (tránh ckpt+model chiếm VRAM gấp đôi; numpy chỉ sống trên CPU). GPU chỉ vào việc từ `.to(device)` và forward. Tăng tốc thật = HF_TOKEN (Secret phải **attach + đọc vào env TRƯỚC from_pretrained** + restart) hoặc đóng model HF thành Kaggle Dataset (hết phụ thuộc mạng).
